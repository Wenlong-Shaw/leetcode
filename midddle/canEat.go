...
func canEat(candiesCount []int, queries [][]int) []bool {
	...
	// 每天至少吃1颗，到favoriteDayi最少吃的数量 min := queries[i][1]+1
	// 每天最多吃daylicapi颗， 到favoriteDayi最多吃的数量 max := queries[i][2]*(queries[i][1]+1)
	for i := 0; i < len(queries); i++ {
		ftype := queries[i][0]
		eatmin := queries[i][1] + 1
		eatmax := queries[i][2] * (queries[i][1] + 1)
		presumleft := 1
		if ftype > 0 {
			presumleft = presum[ftype-1] + 1
		}
		presumright := presum[ftype]
		ans[i] = !(eatmin > presumright || eatmax < presumleft)
	}
	return ans
}
